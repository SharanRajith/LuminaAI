...
async def export_report_latex(req: ExportRequest):
    """Convert report JSON to a downloadable LaTeX (.tex) source file."""
    try:
        data = req.presentation_data
        ...
        lines = [
            ...
            r"\usepackage[colorlinks=true,linkcolor=violet,urlcolor=violet]{hyperref}",
        ...
        return StreamingResponse(
            ...
            headers={"Content-Disposition": f'attachment; filename="{data.get("title","report").replace(" ","_")}.tex"'},
        )
    except Exception as e:
